...
X_train = train_set.drop(config.target, axis=1).select_dtypes(exclude=["datetime64"])
...
mlflow.set_experiment("/Shared/satisfaction_customer-model")
with mlflow.start_run(
    ...
    mlflow.log_param("model_type", "Logistic Regression with preprocessing data")
    mlflow.log_params(config.parameters)

    # Log the model
    signature = infer_signature(model_input=X_train, model_output=model.predict(X_train))
    mlflow.sklearn.log_model(
        sk_model=model, artifact_path="logregression-model", signature=signature
    )
...
model_version = mlflow.register_model(
    model_uri=f"runs:/{run_id}/logregression-model",
    name=model_name,
...
client.set_registered_model_alias(
    name=model_name, alias="latest-model", version=model_version.version
)
...
wrapped_model = SatisfactionCustomerModelWrapper(sklearn_pipeline)  # we pass the loaded model to the wrapper

mlflow.set_experiment(experiment_name="/Shared/satisfaction_customer-pyfunc")
with mlflow.start_run(tags={"branch": "week2", "git_sha": "1234567890abcd"}) as run:
    run_id = run.info.run_id
    signature = infer_signature(model_input=X_train, model_output={"Prediction": "satisfied"})
    conda_env = _mlflow_conda_env(
        additional_conda_deps=None,
        additional_pip_deps=[
            f"code/satisfaction_customer-{__version__}-py3-none-any.whl",
        ],
        additional_conda_channels=None,
    )
    mlflow.pyfunc.log_model(
        python_model=wrapped_model,
        artifact_path="pyfunc-satisfaction_customer-model",
